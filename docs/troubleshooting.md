# Troubleshooting Log — Person B QA Pass

> **Project status (Jul 6, 2026):** Full stack delivery **complete**. All blocking issues below
> are resolved. Item #10 is a cosmetic known limitation only.

Record of bugs found and fixed during end-to-end verification passes of the frontend +
...
Sessions covered: **Jul 3, 2026** (initial pass, items 1–10) · **Jul 6, 2026** (post-merge
verification pass, items 11–13).
...
| 10 | "Registry API" / "Kong Gateway" sidebar lights are hardcoded green | `frontend/app/chat/page.tsx` | ✅ Known limitation (cosmetic) |
| 11 | Access Overview chart bars didn't render on the dashboard | `frontend/app/dashboard/page.tsx` | ✅ Fixed (by teammate, confirmed working) |
| 12 | LangSmith tracing added alongside Jaeger | `agent/runtime_agent.py`, `scripts/verify_langsmith.py`, `.env` | ✅ New — verified working |
| 13 | Transient `OAuthCallback` error on rapid sign-out → sign-in | `frontend/app/api/auth/[...nextauth]/route.ts` (suspected) | 👀 Observed, not confirmed as a real bug |
...
## 10. Known limitation — "Registry API" / "Kong Gateway" sidebar lights are hardcoded green
...
**Status:** **Accepted** — cosmetic only, does not affect clinical workflows or demo readiness.
A future enhancement would use the same pattern as `SystemStatusBadge`: a small
`fetch("/api/registry/servers")` (or a lightweight ping) per row, with loading/error states.
Not required for project completion.

---

## 11. Access Overview chart bars didn't render on the dashboard

**Symptom:** noted during the initial pass — the "Access Overview" bar chart on
`/dashboard` showed day labels (Sat/Mon/Wed/Fri) but no visible bars at all, even with
90+ audit events on record.

**Root cause:** the per-day column `<div>` in `AccessOverview` (`frontend/app/dashboard/page.tsx`)
was `flex-1 flex flex-col items-center gap-1` with no explicit height rule tying it to the
parent's `h-24` — so the percentage-height bar inside it had nothing reliable to size
against.

**Fix (landed in a teammate's `b00ed39` commit, not mine):** added `justify-end h-full` to
the column container, giving each bar a real height to compute its percentage against.

**Verified:** logged in fresh after the Jul 6 merge, opened `/dashboard` — the chart now
renders real blue bars sized to actual daily volume, no code changes needed on my side.

---

## 12. LangSmith tracing — new addition, verified working

Not a bug — a new observability integration that arrived via the Jul 6 merges, verified as
part of the post-merge pass.

**What it adds:** LangSmith traces the LangChain/LangGraph *reasoning* side (which tools the
LLM decided to call, its intermediate steps) — complementary to Jaeger, which traces the
*infrastructure* side (network hops between the agent and each MCP server).

**Config:** `LANGSMITH_API_KEY`, `LANGSMITH_TRACING=true`, `LANGSMITH_PROJECT=patient-risk`
in `.env`. `scripts/verify_langsmith.py` (new) does a live check: confirms config, hits
`/ask`, then queries the LangSmith API for a matching root run.

**Verified:** ran `scripts/verify_langsmith.py` end to end — found a real run in the
`patient-risk` project with `status: success` within a few seconds of the `/ask` call.
Cross-checked the same request in Jaeger too — both systems traced it correctly, at their
respective layers.

**Also confirmed while reading this code:** a teammate finished a gap I'd left in fix #7 —
the *fallback* path in `_run_agent()` (used when a domain has no cached tool schema yet)
previously marked `servers_called` at *connection* time again, same class of bug as #7's
main fix. A new `_wrap_tool_for_tracking()` helper now defers that to actual tool
*invocation* time in the fallback path too, matching the lazy path's behavior.

---

## 13. 👀 Transient `OAuthCallback` error during rapid sign-out → sign-in

**Symptom:** while automating a sign-out (as physician) immediately followed by a sign-in
(as nurse) for RBAC testing, the network trace briefly showed
`GET /?callbackUrl=...&error=OAuthCallback` mid-flow. The session then self-recovered —
the nurse login completed successfully and RBAC behaved correctly afterward.

**Not confirmed as a real bug.** Most likely explanation: signing out and back in within a
couple of seconds — faster than a real clinician would — raced NextAuth's PKCE
`code_verifier` / `state` cookie cleanup from the first session against the second sign-in
attempt's fresh values. Could not isolate a reliable repro; a single manual sign-out/sign-in
cycle with normal human pacing did not reproduce it.

**If this resurfaces:** check `frontend/app/api/auth/[...nextauth]/route.ts` for how the
session cookie is cleared on `signOut()`, and whether Keycloak's SSO logout redirect
(mentioned in the `b00ed39` commit message) fully completes before NextAuth allows a new
`signIn()` to start.
...
- As of Jul 6, 2026, the full stack is **complete and verified**: Docker containers, all 5 MCP
  servers, the runtime agent (with LangSmith tracing enabled), and the frontend dev server.
  If it's been shut down since, resume with: `docker compose up -d` →
  `bash scripts/start_mcp_servers.sh` → `uv run python -m backend.onboarding_agent.register --all`
  → `uv run uvicorn agent.runtime_agent:app --host 0.0.0.0 --port 8500` →
  `cd frontend && npm run dev`. See [`QUICK_TEST.md`](QUICK_TEST.md) for full command reference.
- `audit_events` was truncated (`TRUNCATE audit_events RESTART IDENTITY`) multiple times
  across both sessions to keep the dashboard clean — this is expected and harmless; it does
  not affect clinical data (vitals/labs/meds/diagnoses/notes/radiology), only the
  audit/anomaly history.
- LangSmith is now part of the observability stack alongside Jaeger — see item #12. Both
  were verified working together against the same live request during the Jul 6 pass.
